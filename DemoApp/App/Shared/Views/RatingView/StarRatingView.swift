...
class StarRatingView: UIView {
    ...
    fileprivate let fullStarImage: UIImage = UIImage(systemName: SFSymbol.starFill.rawValue)!
    fileprivate let halfStarImage: UIImage = UIImage(systemName: SFSymbol.starLeadingHalfFilled.rawValue)!
    fileprivate let emptyStarImage: UIImage = UIImage(systemName: SFSymbol.star.rawValue)!
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        self.setupView(rating: 0, color: UIColor.systemOrange, starRounding: .roundToHalfStar)
    }
    ...
    fileprivate func touched(touch: UITouch, moveTouch: Bool) {
        ...
        let touchX = Language.isRTL() ? (hs.frame.width - touch.location(in: hs).x) : touch.location(in: hs).x
